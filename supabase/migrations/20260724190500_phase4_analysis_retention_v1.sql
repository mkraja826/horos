...
declare
  ...
  v_phase4_analysis_cache integer := 0;
...
begin
  ...
  with candidates as (
    select id
    from public.phase4_analysis_cache
    where expires_at < p_now - interval '7 days'
    order by expires_at
    limit p_batch_limit
    for update skip locked
  ), deleted as (
    delete from public.phase4_analysis_cache target
    using candidates
    where target.id = candidates.id
    returning 1
  )
  select count(*)::integer into v_phase4_analysis_cache from deleted;
  ...
  return jsonb_build_object(
    ...
    'phase4AnalysisCacheDeleted', v_phase4_analysis_cache,
    ...
    'retentionPolicy', jsonb_build_object(
      ...
      'phase4AnalysisCacheGraceDays', 7,
